...
class createDataset(object):
    ...
    def _tokenize(self, project, n):
        pre = Preprocessing('large')
        with open('./../data/{}/dataset_{}.pkl'.format(project, n), 'rb') as f:
        ...
        if self.TOKENIZE:
            ...
            for i in tqdm(issues):
                ...
                words = pre.prep(issues[i]['title'], lemma=self.LEMMA, add=False)
                dataset['Title'].at[index] = words
                dataset['TitleLen'].at[index] = len(words)

                body = issues[i]['body']
                if body:
                    words = pre.prep(body, lemma=self.LEMMA, add=True)
                    dataset['Description'].at[index] = words
                    dataset['DescriptionLen'].at[index] = len(words)
                else:
        ...
        if (self.TYPE == 'single') or not(self.TYPE):
            for p in project:
                ...
                if self.OUTPUT:
                    for n in [1]:
                        ...
                        vr.vectorize(project=p, NORM=self.NORM, no_below=2)
            ...
            for p in project:
                ...
                for _p in _project:
                    ...
                    if not(os.path.exists('./../data/{}/cross/{}/title_1gram.csv'.format(p, _p))):
                        header = pd.read_csv('./../data/{}/single/title_1gram.csv'.format(p), chunksize=2)
                        h_col = next(header).columns
                        df = dt.fread('./../data/{}/single/title_1gram.csv'.format(_p)).to_pandas()
                        print('Drop Title Data...')
                        # index = set(df.index) - set(drop_outlier_index)
                        # df = df.query('index in @index')
                        cols = set(h_col) & set(df.columns)
                        df = pd.concat([pd.DataFrame(columns=h_col).astype(int), df.loc[:, cols]], axis=0)
                        df = df.fillna(0)
                        df.to_csv('./../data/{}/cross/{}/title_1gram.csv'.format(p, _p), index=None)
                        print('-> Finished')
                        del df
                        gc.collect()
                    
                    if not(os.path.exists('./../data/{}/cross/{}/body_1gram.csv'.format(p, _p))):
                        header = pd.read_csv('./../data/{}/single/body_1gram.csv'.format(p), chunksize=2)
                        h_col = next(header).columns
                        df = dt.fread('./../data/{}/single/body_1gram.csv'.format(_p)).to_pandas()
                        ...
                        df = pd.concat([pd.DataFrame(columns=h_col).astype(int), df.loc[:, cols]], axis=0)
                        df = df.fillna(0)
                        df.to_csv('./../data/{}/cross/{}/body_1gram.csv'.format(p, _p), index=None)
                        print('-> Finished')
                        del df
                        gc.collect()
...
if __name__ == '__main__':
    ...
    cd.main(project=['vscode', 'influxdb', 'saleor', 'server'])